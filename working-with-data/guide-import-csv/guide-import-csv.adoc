...
* Make sure to set the heap size so a sufficient value.
You do this by setting `dbms.memory.heap.initial_size` and `dbms.memory.heap.max_size` to at least 4G in `neo4j.conf`.
If using cypher-shell, the memory is set by doing `export JAVA_OPTS="-Xmx4G"` before starting cypher-shell.
...
`LOAD CSV` is great for importing small - medium sized data, i.e. up to the 10B records range.
