...
As you may have found in reading this guide or in your own experience with graph data modeling, there is no right or wrong way to model your data.
...
In contrast, finding similar class offerings to a current course might work well with a high-level category hierarchy that makes searching all classes within a subject topic more efficient.
...
As we talked about when using multiple models in your graph, there are tradeoffs.
...
As mentioned in paragraphs above, changes are always possible with Neo4j.
...
Cypher allows you to write queries to run mass updates across labels, add/remove properties, or insert additional nodes and relationships into the structure.
