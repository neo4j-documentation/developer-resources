...
[role=side-nav]
* link:{opsmanual}/introduction/#_high_availability[Neo4j High Availability,role=docs]
...
Details on the arbiter are available in the link:{opsmanual}/deployment/#arbiter-instances[Neo4j Manual].
...
See the link:{opsmanual}/deployment/#ha-configuration[documentation] on `ha.slave_only`.
