...
An in-depth description on this topic is available in the link:{opsmanual}/#memory-tuning[Neo4j Operations Manual].
...
For a more thorough discussion on this topic, refer to the link:{opsmanual}#heap-sizing[heap memory configuration section in the Neo4j Manual].
...
To set the value system wide you have to follow the link:{opsmanual}#linux-open-files[instructions] for your platform.
