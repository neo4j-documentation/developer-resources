...
Let us look at an example to help understand this better.
For instance, we have data on people stored in our system.
...
****
[NOTE]
There are a variety of use cases where we might want to shard.
Those are covered link:/developer/neo4j-fabric-sharding/#why-sharding[later in this guide].
****

Here is an example of our data as rows and columns (typical relational database structure).
However, we could format this data as individual documents, entities, or any other database structure.

image::{img}concept_fabric_example_data.jpg[link="{img}concept_fabric_example_data.jpg",role="popup-link"]

This is only a very small sample of the data stored in our system, but we want to divide all the records of people up into shards.
We can choose to do this in a few different ways.
...
3. Other - based on any other data we might collect, such as job role, year of system entry, residency status, number of connections to others, etc.

An example of the first two options is shown below.
...
The concept of sharding in the property graph model takes on a slightly different approach than with other database models, which we will discuss link:/developer/neo4j-fabric-sharding/#graph-sharding[later in this guide].
...
* To minimize latency of queries in various regions, relevant segments of the graph can be stored in cloud regions close to query request sources (i.e. application hosted in Europe queries for data stored in a Europe cloud region).
...
* The graph size is becoming large enough (tens of billions of nodes) that it makes sense to divide the data into smaller graphs to run on smaller-sized hardware and accessed by necessary parties.

There are other possible scenarios where sharding the data may be the solution of choice, but this gives us some ideas where it most commonly useful.
At the core, sharding provides a way to distribute data across instances or geographic regions for easier or more secure access.
...
Another factor involved is data connectedness - the more data is connected, the more complicated it is to implement sharding.
...
Neo4j has found a solution to approach graph sharding, and we will discuss that in more detail in the next section.

[#neo4j-fabric]
== What is Neo4j Fabric?

Neo4j Fabric is Neo4j's solution to graph sharding by breaking a larger graph down into individual, smaller graphs and storing them in separate databases.
For graphs that are highly-connected, this means a basic level of data redundancy to bridge relationships across shards.
...
For this example, we will choose to shard the data by continent and create a separate database for each continent.
...
No matter the location of the DBMS, we have sharded our data into 1 instance for the fabric database and separate instances for each of continents containing its people data.
...
The shards are still organized into 1 for the coordinator (fabric db) and 6 instances for the people data based on continent, but the difference is that all of these instances are now bundled or separated into separate systems.

image::{img}concept_fabric_example_3dbms.jpg[link="{img}concept_fabric_example_3dbms.jpg",role="popup-link"]

This architecture might be chosen because certain data might be required to be hosted privately or separately from other data or because the bulk of requests are for a particular dataset.
...
When we start discussing clustering, especially for certain DBMSs and not for others, it can sometimes get complex and confusing.
...
No matter the combination of requirements, Neo4j Fabric can support a solution to suit various needs.
...
Sharding meant dividing that connected unit into many graphs and retaining the value of the relationships across instances and clusters, as well as maintaining data consistency and integrity.
...
Determining the best way to divide the graph data into separate graphs for Fabric can be trickier than you might imagine, especially if the data is tightly connected.
...
In our people data example, we have clean divisions in the data by continent.
...
Even so, the data duplication for this (the person's data existing in each of the continents they are citizens) is minimal.
If our continent data is based on birth, however, it eliminates this, as a person cannot be born in more than one continent.

Understanding the context and data definitions, then, could also be important to making the best decision on where to divide the data. Another perspective to this is how the data is modeled, as this can impact whether clear subgraphs in the data naturally occur based on the model.
...
As an alternative, separating by first letter of name could mean we end up with 26 potential databases or separating by country could mean around 200 potential databases (imagine if we needed to replicate these!).
...
Knowing ahead of time the types of queries that will be necessary of the systems or the kinds of requests that are most common will help plan a data model that best suits the performance requirements of those queries and optimize for them.
Our next paragraphs will take a closer look at this.

=== Query structure and optimization

Planning ahead by understanding the query requirements will help constructing a data model and architecture that best optimizes for those expected requests.
We cannot always plan every request that may be asked of the system, but listing the expected ones and drafting up those example queries can make a big difference in building a system that is designed to handle what is needed.
...
Requirements may be completely different in your system architecture and may require a different structure.

Understanding what the load will be and the processing power needed to handle the data in a timely manner can make a big difference in the success of the system.
...
****
[NOTE]
Want to know more about querying with Neo4j Fabric?
We will dive into this topic deeper in an upcoming guide!
****
