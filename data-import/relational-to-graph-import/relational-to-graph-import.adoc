...
*5)* Other ETL tools: there are also a few vendor and community tools available for similar etl processes and GUI interaction for getting data from various sources into and out of Neo4j.

*6)* link:/developer/language-guides/[Programmatic via drivers]: ability to retrieve data from a relational data and use the bolt protocol to write it to Neo4j through one of the drivers with your programming language of choice.
