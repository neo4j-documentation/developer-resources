...
In this guide, we will work with a zipped folder containing three CSV files and import the data to Neo4j with Neo4j Desktop.
...
When you start the application, you should see something like the screenshot below.
...
Choose the `Create a Local Graph` button and fill in details on the entry form for your database.
You can leave whatever version is defaulted for the last dropdown.
...
`LOAD CSV` is a built-in command in Cypher that allows us to read CSV files and append regular Cypher statements to create the data as a graph.
...
Since all of these files have a decent numbers of rows, we also use `LIMIT` to get a sample.
...
Though the sample values from our inspection above are all whole numbers, we know that monetary amounts can have decimal place values, so we will convert these values to floats using the `toFloat()` function.
...
[NOTE]
--
You can try re-running the load query above to see your constraints in action.
Running it again should not allow you to insert duplicate data and fail with a constraint error.
--
...
LOAD CSV WITH HEADERS FROM 'file:///desktop-csv-import/orders.csv' AS row
WITH toInteger(row.orderID) AS orderId, datetime(replace(row.orderDate,' ','T')) AS orderDate, row.shipCountry AS country
CREATE (o:Order {orderId: orderId})
...
CREATE (o)-[rel:CONTAINS {quantityOrdered: quantityOrdered}]->(p)
RETURN count(rel)
----
...
To learn more about `LOAD CSV` and its uses, check out our guide on link:/developer/data-import/guide-import-csv/[LOAD CSV].
