...
* link:/developer/cypher/cypher-query-language/[Cypher]
* link:/developer/data-modeling/guide-data-modeling/[Property Graph Data Model]

You should also have downloaded link:https://neo4j.com/download/[Neo4j Desktop^] and be familiar with navigating the application, as well as creating a starting a database.
...
Neo4j Desktop provides a more user-friendly interface than other methods for starting and creating Neo4j databases, adding or removing plugins, changing configurations, and other functionality.
It also includes some shortcuts and easy access for importing flat files (such as CSVs) into a Neo4j instance.

In this guide, we will work with a zipped folder containing three CSV files and import the data to Neo4j using Neo4j Desktop.
Our CSV files contain data for products, orders, and order line items.
We will look at the exact information in the files in an upcoming paragraph.
...
We need to open the Neo4j Desktop application on your computer to begin.
When you start the application, you should see something similar to the screenshot below.

image::{img}desktop_mainscreen.jpg[link="{img}desktop_mainscreen.jpg",role="popup-link"]

You may already have a database instance or two from other projects or exercises, but let's create a separate database for this import.
To do that, you click on the section that says `Add Graph` in the image above.
When you do that, you will see two buttons appear for creating a local graph or connecting to a remote graph.

image::{img}desktop_add_graph.jpg[link="{img}desktop_add_graph.jpg",role="popup-link"]

Choose the `Create a Local Graph` button, and fill in some details on the entry form for your database.
You can leave whatever version is defaulted in the last dropdown.
...
When it starts, the button should change to `Stop` so you now see the screen below.
...
Now that our files are in the import folder, we can look at how to put the data from the files into our database.
We will need to take the current table and column format in the CSV files and translate them into nodes and relationships for our graph model.
This can be done a few different ways, but we will use Cypher's `LOAD CSV` command in this guide.

== LOAD CSV

`LOAD CSV` is a built-in command in Cypher that allows us to read CSV files and append regular Cypher statements to translate the data to our property graph data model.
You can also use `LOAD CSV` without creating the graph structure and just output samples, counts, or distributions.
...
Before we insert anything into our graph database, let us take a look at the data in the csv files.
...
Since all of these files have a decent numbers of rows, we also use `LIMIT` to just get a sample of the whole file.
...
Product ID looks to be an integer value that increases with each row, so we can convert this to an integer using the `toInteger()` function in Cypher.
Product name can remain a string because it consists of characters.
The final column is the product unit cost.
Though the sample values from our inspection above are all whole numbers, we know that monetary amounts can have decimal place values, as well, so we will convert these values to floats using the `toFloat()` function.
...
----
LOAD CSV FROM 'file:///desktop-csv-import/products.csv' AS row
RETURN toInteger(row[0]), row[1], toFloat(row[2])
LIMIT 3
----

image::{img}desktop_import_products_convert.jpg[link="{img}desktop_import_products_convert.jpg",role="popup-link"]

Note that we are using collection positions (row[0], row[1], row[2]) to refer to the columns in the row.
In a file that has no headers, this is the way to reference values in each position.
When we look at the next two files that have headers, you will see that we can use the column name to reference the value.
...
OrderID looks like it is an integer, so we can convert that using the `toInteger()` function.
...
----
LOAD CSV WITH HEADERS FROM 'file:///desktop-csv-import/orders.csv' AS row
RETURN toInteger(row.orderID), datetime(replace(row.orderDate,' ','T')), row.shipCountry
LIMIT 5
----
...
Neo4j's datetime format will accept many formats, but the time value must start with a `T`.
The CSV file did not have the 'T' joining the date and time values but had a space between them instead.
...
Let us look at which ones will need to be converted.
...
----
LOAD CSV WITH HEADERS FROM 'file:///desktop-csv-import/order-details.csv' AS row
RETURN toInteger(row.productID), toInteger(row.orderID), toInteger(row.quantity)
LIMIT 8
----
...
We do not need to analyze all the various ways we could organize the products and orders in our files, so we have provided the model we will use for this exercise below.
...
Each of those nodes have properties that we have from our CSV files.
...
In this exercise, we will add two constraints - one for `productId` and one for `orderId`.
This will ensure when we create a new node of each of those types or a relationship to connect them, we know the entities are unique and indexed.

Good practice is to add any constraints and indexes before loading to avoid existing violations and ensure data is added to the index as it is created.
...
Now we are ready to create the Cypher for creating the data in the graph!
We will use `CREATE` statements where we are sure that we will not have duplicate rows in our CSV file.
For values we know exist in the database already, we will first use `MATCH` to find them before executing any updates or inserts.
...
We can use the `CREATE` statement, as we feel confident that we have our constraint in place, no duplicate rows in our file, and no existing data in our graph.
The properties will be set to the converted values we handled earlier in this guide.

[source, cypher]
----
LOAD CSV FROM 'file:///desktop-csv-import/products.csv' AS row
CREATE (p:Product {productId: toInteger(row[0])})
  SET p.productName = row[1], p.unitCost = toFloat(row[2])
RETURN count(p)
----
...
Again, since we feel confident our constraint will catch violations and our CSV file data has no duplicates, we can use the `CREATE` statement to insert the data more efficiently.
Just as with products, we start with the `LOAD CSV` command, then add Cypher statements and include our data conversions.

[source, cypher]
----
LOAD CSV WITH HEADERS FROM 'file:///desktop-csv-import/orders.csv' AS row
CREATE (o:Order {orderId: toInteger(row.orderID)})
  SET o.orderDateTime = datetime(replace(row.orderDate,' ','T')), o.shipCountry = row.shipCountry
RETURN count(o)
----
...
Then, since we know relationships do not yet exist in the graph, the `CREATE` statement will add the new relationship.
...
CREATE (o)-[rel:CONTAINS {quantityOrdered: toInteger(row.quantity)}]->(p)
RETURN count(rel)
----
...
[source, cypher]
----
MATCH (p:Product)-[rel:CONTAINS]-(o:Order)
