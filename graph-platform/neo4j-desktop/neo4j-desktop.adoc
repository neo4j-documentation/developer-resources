...
To remove the project, hover over the `Project` row in the projects menu and click the trash bin icon that appears on the right side (image:{img}delete_project_icon.jpg[width=20]).
...
The certificate icon (image:{img}activation_keys_icon.png[width=20]) shows the licenses that are attached to this installation in Neo4j Desktop, which includes activation of Desktop itself along with any graph app licenses.
...
image::{img}db_section.jpg[link="{img}db_section.jpg",role="popup-link"]
...
image::{img}manage_db.jpg[link="{img}manage_db.jpg",role="popup-link"]
...
image::{img}db_details_tab.jpg[link="{img}db_details.jpg",role="popup-link"]
