...
If activated, Neo4j Desktop should show it in the list of graph applications like the image below.
...
We will use the Northwind data, as this outlines a retail system with orders, products, and customers.

With Bloom, we will see how to navigate and explore the data to gather business value and insights.

To load the data, we can start our instance in Neo4j Desktop and open Neo4j Browser by going to `localhost:7474` in a web browser.
...
More information on different perspectives and customizing is found in the link:/docs/bloom-user-guide/current/perspectives/[documentation^].
...
We can zoom in closer to a section of the results by using the trackpad on our laptop or mouse or by using the `+` and `-` buttons in the lower left of the scene.
...
For our example, we will choose to show the `productName` property, so check the box next to that property for `Caption` and then uncheck the `Caption` checkbox for the `productID` property.
...
Finally, we can check that everything worked by closing the perspective panel and looking at our scene again.
...
All of the nodes match the colors from the legend, so we have some red nodes that are orders (28 of those), an orange node that is a supplier (1 of those), and category node that is a product category (1 of those).
...
Bloom makes suggestions based on what it finds in the graph, so you don't have to type it out.
When you pick from the search suggestions, you can choose which relationships are traversed.
...
Feel free to explore others, as well, such as suppliers `Pavlova` and `New Orleans Cajun Delights`.

[#bloom-order-search]
=== Finding orders with two products
Let us see if we can find one more complex pattern for finding customers who ordered two products together.
We can choose anything, but let's start with the products `Ipoh Coffee` and `Teatime Chocalate Biscuits`.
As we saw in our previous search, the supplier `Specialty Biscuits` makes the chocolate biscuits, but doesn't make the coffee, so it might be less likely to be ordered together.

For this type of query, we can use Bloom capability to search indexed properties to shorten our query.
Instead of searching for a product with a particular name, as long as there is an index on the property in the database, we can just type in the property value, and Bloom will find the related node category.

This is the search phrase we can use to answer our question.

.Search
`Customer Order Teatime Chocolate Biscuits and Ipoh Coffee`
