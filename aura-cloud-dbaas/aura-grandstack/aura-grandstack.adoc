...
Now that we've created our Neo4j Aura database, we're going to learn how to build a GRANDstack application on top of it.
We're going to
...
Now click on the `Check existing database` button.
...
From here we can import databases or dump file.
...
== The GRANDstack application

We're now ready to look at our GRANDstack application.
...
This code powers a GraphQL API on top of our Neo4j database, and lives in the `api` directory.
