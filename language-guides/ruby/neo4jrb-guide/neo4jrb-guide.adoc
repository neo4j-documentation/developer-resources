...
Here you created your own application, but if you would like to play with a more developed application check out our link:https://github.com/neo4j-examples/asset_portal[asset_portal] app.  The project introduces a single `AssetController` to avoid the duplication from this guide and also uses other tools like link:http://semantic-ui.com/[Semantic UI] for a cleaner interface.
