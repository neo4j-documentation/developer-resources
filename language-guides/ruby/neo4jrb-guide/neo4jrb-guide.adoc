...
For this you'll need to add a new association to your `Category` model:

.app/models/category.rb
[source,ruby]
----
has_many :in, :books, origin: :category
----

With this it is relatively simple to get a start on querying potential recommendations.  Try running this bit of Ruby code in your Rails console:

[source,ruby]
----
Book.all.category.books.to_a
----
...
What this is doing is finding all of the books that share categories with all other books.  This isn't particularly useful until we start introducing some variables.  What if we wanted to find out how many books share a category with every book?

[source,ruby]
----
Book.as(:book).category.books.pluck('book', 'count(*)')
----

What if we wanted to find out what those books are?

[source,ruby]
----
Book.as(:book).category.books(:other_book).pluck('book', 'collect(other_book)')
----

Taking it a step further, let's create a query which finds any book which shares at least two categories (NOT POSSIBLE, WTF?)
...
@recommendations = Book.as(:book).
                    ...
                    query.
                    with('book, collect(other_book) AS other_books').
                    where('length(other_books) > 1').
                    pluck('book.isbn', 'other_books')
