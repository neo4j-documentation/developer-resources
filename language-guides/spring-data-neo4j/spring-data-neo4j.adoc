...
However, if you want to benefit from full-fledged object mapping and the other helpful support that comes with Spring Data, you can use *Spring Data Neo4j*.
...
Here, we are using Maven, so pom.xml is the dependency file.
...
The first few lines of each domain class is shown below.
...
On the `ACTED_IN` relationship, we store a relationship property that contains a list of the role(s) that the person played in that movie.
...
The `Role` class also has an _id_ field that is managed by the database and a _list_ field to contain the possible roles that a person could play in a movie (could be more than one).
...
The direction property is _outgoing by default_, so we must specify that the relationships are incoming on the `Movie` node.
...
The `@JsonIgnoreProperties` annotation to the relationship variables to ignore the relationship variables on the next entity, avoiding infinite recursion errors and duplicate information returning.
...
The interfaces extend the `Neo4jRepository`, which extend the Spring CRUDRepository for persisting and retrieving data.
...
The custom query we defined using Cypher will retrieve the `Person` results, along with that individual's relationships and movies.
...
We also provide reactive development through the Spring Data Neo4j RX project.
More information is provided in the link:/developer/spring-data-neo4j-rx/[SDN-RX guide].
