...
If you've link:/download[installed] and started Neo4j as a server on your system, you can interact with the database via the built-in Neo4j Browser application on http://localhost:7474[localhost:7474].
...
A simple HTTP Cypher request, executable in the Neo4j Browser would look like:

[source,json]
----
:POST /db/data/transaction/commit {"statements":[
      {"statement":"CREATE (p:Person {firstName:{name}}) RETURN p", "parameters":{"name":"Daniel"}}
    ]}
----

The drivers use the same APIs under the hood, but make them available in a convenient way.
