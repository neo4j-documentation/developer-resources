...
But regardless of the source we typically won't be lucky enough to have our data already structured as a graph so we'll first need to design a graph model - you can learn how to do that in our link:build-a-graph-data-model/guide-intro-to-graph-modeling[modeling guide]
