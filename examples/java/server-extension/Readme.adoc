...
Spark is a micro web framework used to easily define routes for endpoints and provide their implementation.
In our case, the implementation calls the `MovieService` which has one method per endpoint that returns Java collections which are turned into JSON using Gson.
