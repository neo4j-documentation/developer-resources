...
[NOTE]
This project is currently beta quality and not officially supported by Neo Technology. Not for use in production.
...
For alternative installation workflow, see the documentation link:https://github.com/neo4j-contrib/neo4j_doc_manager/blob/master/docs/neo4j_doc_manager_doc.adoc[here].
...
To see all configuration options for Neo4j Doc Manager (including specifying which collections and fields to synchronize) see the documentation link:https://github.com/neo4j-contrib/neo4j_doc_manager/blob/master/docs/neo4j_doc_manager_doc.adoc[here].
...
image::doc_to_graph.png[Document to graph]

To see this in action, insert the following document into MongoDB using the mongo-shell:
...
This document is converted to a property graph and inserted into Neo4j with this structure:

image::prop_graph.png[Property graph]
