...
type Movie {
    ...
    recommendation(first:Int = 3): [Movie] @cypher(statement:"")
}
...
image::https://dl.dropboxusercontent.com/u/14493611/neo4j-graphql.png[width=200,float=right]
...
You can grab the latest release our extension from https://github.com/neo4j-graphql/neo4j-graphql/releases[here] and drop it into any Neo4j 3.1.x server (`plugins` directory).
...
We're especially proud of the `@cypher` directive, the auto-generated mutations, and the `graphql.execute` and `graphql.schema` procedures.
...
type Movie {
    ...
    recommendation(first:Int = 3): [Movie] @cypher(statement:"MATCH (this)<-[r1:REVIEWED]-(:User)-[r2:REVIEWED]->(reco:Movie) WHERE 3 <= r1.stars <= r2.stars RETURN reco, sum(r2.stars) as rating ORDER BY rating DESC")
}
...
type Mutations {
    directed(movie:ID! director:ID!) : String @cypher(statement:"MATCH (m:Movie {title: $movie}), (d:Person {name: $director}) MERGE (d)-[:DIRECTED]->(m)")
}
schema {
   mutations: Mutations
}
...
image::https://image.slidesharecdn.com/discoveringdarkdatajavazone-2015-09-09-150910133613-lva1-app6891/95/graph-search-and-discovery-for-your-dark-data-19-638.jpg?cb=1441892388[width=600]
...
GraphQL is defined as a https://facebook.github.io/graphql/[specification^] with lots of detailed resources available.
...
We found it very helpful and inspiring to bring the people and activities in all the different channels together into one 360° view.
...
The data is publicly accessible here (http://107.170.69.23:7474 username/pwd: graphql/graphql).

image::https://github.com/community-graph/graphql-community/raw/master/images/community-graphql-cypher-graph.jpg[width=600]

If you are used to Neo4j you can use it right away by writing some link:/developer/cypher[Cypher^] statements (https://github.com/community-graph/dashboard[some suggestions^]).
...
Write an demo / application / dashboard / visualization / timeline / react component, whatever you want against this backend.
