...
.Goals
[abstract]
This article demonstrates how to use Neo4j for desktop for managing instances of Neo4j locally for development.
...
If this is the first time using Neo4j for desktop, the default `My Project` will be the only one.
...
The app will add a row in the menu for a project named `Project`. You can change the name by clicking on the pencil icon that appears when you hover on the project name in the right pane.
...
Application settings can be modified or viewed by clicking the gear towards the bottom of the left-side menu in Neo4j for desktop (image:{img}settings_icon.png[width=20]).
...
It shows who is currently logged into Neo4j for desktop.
...
Next, you will type in a name for your database and set a password and choose a version (default is latest version), then click kbd:[Create].
...
The buttons next to those allow you to open the folder location of the database (for modifying data files and configuration) and to open the Neo4j Browser to interact with data in the database.
...
Currently, Neo4j has plugins for APOC, GraphQL, and Graph Algorithms.
Short descriptions of each are shown in the Neo4j for desktop pane.
...
If you have feedback or questions on how to use Neo4j for desktop, feel free to reach out to us.
You can submit messages to us through Intercom and tag as related to `desktop`.

== Next Steps

Now that we covered the basics of Neo4j for desktop, you can start working with data using our query language, link:/developer/cypher[Cypher^].
