...
Very simply, a graph database is a database designed for storing and retrieving data and the relationships among it.
Rather than prioritizing a rigidly-formatted structure to force on the data and navigating the structure to analyze sets of data, a graph database directly connects each piece of data to those it is related to, elevating the relationships in the data model to equal importance with the data itself.

++++
<iframe width="680" height="425" src="https://www.youtube.com/embed/_D19h5s73Co" frameborder="0" allowfullscreen></iframe>
++++
...
With only with a pattern and a set of starting points, graph databases explore the larger neighborhood around those initial starting points -- collecting and aggregating information from millions of nodes and relationships -- and leaving any data outside the search perimeter untouched.


[[property-graph]]
== The Property Graph Model

As with most technologies, there are few different approaches to how the data is organized and what makes up the key components of a graph database.
One such approach is the property graph model, where data is organized as nodes, relationships, and properties (metadata stored on the nodes or relationships).
...
In contrast to graph processing or in-memory libraries, Neo4j also provides full database characteristics, including ACID transaction compliance, cluster support, and runtime failover - making it suitable to use graph data in production scenarios.
